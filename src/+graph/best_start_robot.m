function best_n = best_start_robot(g, start, target, obstacles, partition, robot)
    ...
    if i <= 0
        ...
        for j = 1:size(dist)
            width_start = corridors.edge_width(start, vertices(j,:), obstacles);
            width_target = corridors.edge_width(vertices(j,:), target, obstacles);
            if robot_radius > max(width_start,width_target)
                dist(j) = +inf;
            end
        end
